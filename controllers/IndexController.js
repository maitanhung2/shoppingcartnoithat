...
class IndexController {
  ...
  postUpdateQTYInCart(req, res, next) {
    ...
    customers
      .updateOne(
        { "loginInformation.userName": user, "listProduct.productID": id },
        { $set: { "listProduct.$.amount": quantity } }
      )
      .then(() => {
        res.redirect("/cart");
      })
      ...
      customers.findOne(
        ...
        (err, customerResult) => {
          region.findOne({ Id: city }, (err, cityResult) => {
            ...
            var districtData = cityResult.Districts.filter(
              (e) => e.Id == district
            );
            var districtName = districtData[0].Name;
            var wardName = districtData[0].Wards.filter((e) => e.Id == ward)[0]
              .Name;
            var data = {
  ...
  getDeleteFavorite(req, res, next) {
    if (req.isAuthenticated()) {
      ...
      customers
        .updateMany(
          { "loginInformation.userName": user },
          { $pull: { listFavorite: { _id: OjectId(id) } } }
        )
        .then(() => {
